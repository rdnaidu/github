...
3. `\(.+)\)` leaves clear I need to capture all the content left, excepts the closing parenthesis; thats why its outside the group catcher.
...
These tests for catching true negatives
...
And these others to catch the true positives:
...
So far so good, but in the previous implementaton for catching the groups we are using numeric indexes. Its not so bad, but by simply naming groups we could turn the catching of values more readable. For instance, it allows to skip the temporary variables while being clear about which parameter of the constructor we are passing. 
...
## Conclusions
Regular expressions are a powerful tool that can be used in almost any modern language. Used for parsing can be tricky to be understood, but with the help of named groups we can turn it more meanful for yourself in the future and other developers.
